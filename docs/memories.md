...
Memories are local, searchable notes attached to AgentHub. They are useful for decisions you want to keep near a project, conventions for one agent, or a short global preference. They are not a background prompt store: AgentHub never injects them into Claude Code, Codex or another agent automatically.

Open **Memories** from the sidebar and choose **New memory**. Give the note a title and body, then choose a scope:

- **Global** applies everywhere in AgentHub.
