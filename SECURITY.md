...
- **Marketplace content is untrusted.** Browsing is opt-in and uses public GitHub endpoints. AgentHub shows exact files and warnings before installation. It never runs scripts, starts MCP processes, changes shell profiles or silently modifies an agent directory.
...
The desktop app has no telemetry, account or cloud backend. A user can choose to browse public GitHub sources, but local transcripts, memories and configuration contents are not uploaded by AgentHub.
